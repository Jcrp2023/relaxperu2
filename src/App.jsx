...
import { categories, cities, filterActivities, limaDate, safeSourceUrl, upcomingDate } from './catalog';
...
const copy = {
  es: {
    ...
    intro: 'Conciertos, cultura, escapadas y momentos para desconectar. Descubre actividades en el Perú desde un solo lugar.', search: 'Busca un concierto, lugar o experiencia', city: 'Ciudad', date: 'Fecha',
    ...
    update: 'Catálogo editorial: la información puede cambiar.', legal: 'RelaxPerú ayuda a descubrir planes. La compra o reserva se realiza con el proveedor enlazado; comprueba su identidad, condiciones y medios de pago.',
  ...
  en: {
    ...
    intro: 'Concerts, culture, short trips and moments to unwind. Discover activities in Peru from one place.', search: 'Search concerts, places or experiences', city: 'City', date: 'Date',
    ...
    update: 'Editorial catalog: information can change.', legal: 'RelaxPerú helps you discover plans. Purchases and bookings take place with the linked provider; check its identity, terms and payment methods.',
...
export default function App() {
  ...
  const visible = useMemo(() => filterActivities(activities, { search, city, category, when, favoritesOnly, favorites }, today), [search, city, category, when, favoritesOnly, favorites, today]);
  ...
  function reset() { setSearch(''); setCity('all'); setCategory('all'); setWhen('all'); setFavoritesOnly(false); }
  ...
  return <>
    ...
    <main><section className="hero"><div className="container hero-grid"><div className="hero-copy"><p className="eyebrow"><span className="eyebrow-dot"/>{t.eyebrow}</p><h1>{t.heading}<br/><em>{t.highlight}</em></h1><p className="hero-intro">{t.intro}</p><a className="primary-button" href="#planes">{t.explore} <span aria-hidden="true">→</span></a><div className="hero-proof"><span className="proof-icons">✦ ✳ ✺</span><span>{t.trust1}</span></div></div><div className="hero-visual" aria-hidden="true"><div className="visual-sun"/><div className="visual-arch"><span className="arch-hill arch-one"/><span className="arch-hill arch-two"/><span className="arch-path"/><span className="arch-sun"/></div><div className="floating-tag tag-top">✳ &nbsp; {t.wellness}</div><div className="floating-tag tag-bottom">✦ &nbsp; {t.interest}</div><span className="visual-star star-one">✳</span><span className="visual-star star-two">✺</span></div></div></section>
    <section id="planes" className="catalog container"><div className="catalog-top"><div><p className="eyebrow small">{t.section}</p><h2>{favoritesOnly ? t.favorites : t.featured}<span className="heading-dot">.</span></h2></div><p className="catalog-subtitle">{t.trustText}</p></div><div className="filter-panel"><label className="search-field"><span className="sr-only">{t.search}</span><span aria-hidden="true">⌕</span><input value={search} onChange={event => setSearch(event.target.value)} placeholder={t.search}/>{search && <button onClick={() => setSearch('')} aria-label={t.clear}>×</button>}</label><label className="select-field"><span aria-hidden="true">⌖</span><span className="sr-only">{t.city}</span><select value={city} onChange={event => setCity(event.target.value)}><option value="all">{t.allCities}</option>{cities.slice(1).map(value => <option key={value} value={value}>{value === 'Ica' ? 'Ica / Pisco' : value}</option>)}</select></label><label className="select-field"><span aria-hidden="true">◷</span><span className="sr-only">{t.date}</span><select value={when} onChange={event => setWhen(event.target.value)}><option value="all">{t.anytime}</option><option value="today">{t.today}</option><option value="week">{t.week}</option><option value="explore">{t.exploreAnytime}</option></select></label></div><div className="category-list" aria-label={t.interest}>{categories.map(value => <button key={value} className={category === value ? 'category active' : 'category'} onClick={() => setCategory(value)} aria-pressed={category === value}><span aria-hidden="true">{glyphs[value]}</span>{t[value]}</button>)}</div><div className="results-header"><p><strong>{visible.length}</strong> {t.results}</p>{favoritesOnly && <button className="text-button" onClick={() => setFavoritesOnly(false)}>{t.back}</button>}</div>{visible.length ? <div className="cards-grid">{visible.map(item => <Card key={item.id} item={item}/>)}</div> : <div className="empty-state"><span className="empty-icon">{favoritesOnly ? '♡' : '⌕'}</span><h3>{favoritesOnly && favorites.length === 0 ? t.savedEmpty : t.noResults}</h3><p>{favoritesOnly && favorites.length === 0 ? t.savedEmptyText : t.noResultsText}</p><button className="outline-button" onClick={reset}>{t.clear} →</button></div>}</section>
